...
int32_t t_fine; // Global variable for fine temperature used in pressure calculation
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Calibration data T: %d, %d, %d\n", dig_T1, dig_T2, dig_T3);
    ...
    ESP_LOGI(TAG, "Calibration data P: %d, %d, %d... \n", dig_P1, dig_P2, dig_P3);

    while (true) {
        // Read Temperature
		ESP_ERROR_CHECK(bmp280_register_read( dev_handle, BMP280_REG_TEMP_MSB, aRxBuffer, BMP280_TEMP_REG_SIZE));
		temp_raw = (aRxBuffer[0] << 12) | (aRxBuffer[1] << 4) | (aRxBuffer[2] >> 4);
        
        // Read Pressure
        uint32_t press_raw;
        ESP_ERROR_CHECK(bmp280_register_read(dev_handle, BMP280_REG_PRESS_MSB, aRxBuffer, BMP280_PRESS_REG_SIZE));
        press_raw = (aRxBuffer[0] << 12) | (aRxBuffer[1] << 4) | (aRxBuffer[2] >> 4);

        // Calculate and Print
        double temperature = bmp280_compensate_T_double(temp_raw);
        double pressure = bmp280_compensate_P_double(press_raw);

        ESP_LOGI(TAG, "Temperature: %.2f (+/- 0.1) C | Pressure: %.2f (+/- 1) hPa", temperature, pressure/100.0);
		sleep(1);
    }
}
